...
packages_install() {
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt install -y $1 > /dev/null 2>&1
    if [ $? -eq 0 ]; then
...
while IFS= read -r line; do
    if [[ $line == nameserver* ]]; then
        echo "${line%$'\n'} ${ip_address}:0.0" >> "$temp_file"
    else
        echo "$line" >> "$temp_file"
    fi
done < "$resolv_conf"

sudo mv "$temp_file" "$resolv_conf"
...
add_lines_to_file() {
    ...
    if [ -f "$file_path" ]; then
        ...
        echo "Les lignes ont été ajoutées à $file_path"
        return 0
    else
